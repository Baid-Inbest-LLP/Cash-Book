...
import { useExpenseHeadReport, useExportExpenseHeads } from '../../hooks/useReports';
...
import { CompanySelect, FinancialYearSelect, MonthSelect } from './ReportFilters';
...
export default function ExpenseHeadReportPage() {
  ...
  const { data, isLoading, isError, error: queryError } = useExpenseHeadReport(params);
  const rows = data?.expenseHeads ?? [];
  const error = isError ? getApiErrorMessage(queryError, 'Failed to fetch expense-head report') : null;

  const exportReport = useExportExpenseHeads();

  const columns = [
    { key: 'expenseHead', header: 'Expense Head', render: (row) => row.expenseHead?.name || '-' },
    {
      key: 'paymentAmount',
