...
export default function ExpenseHeadForm({ expenseHead, onClose }) {
	...
	const submitting =
		createExpenseHead.isPending || updateExpenseHead.isPending;
	...
	const onSubmit = async (data) => {
		...
		try {
			if (isEdit) {
				await updateExpenseHead.mutateAsync({
					id: expenseHead._id,
					data: payload,
				});
			} else {
			...
			>
				<div className="company-form-header">
					<div>
						...
						<p className="company-form-subtitle">
							{isEdit
								? "Update the expense head name or status"
				...
				>
					...
					<div className="flex items-center gap-2">
						...
						/>
						<label
							htmlFor="expenseHeadActive"
							className="company-form-checkbox-label cursor-pointer"
						>
							Active
						</label>
					</div>
